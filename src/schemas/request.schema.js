...
export default {
  requestSchema: [
    check('status')
      .custom((value) => {
        if (value) {
          throw new Error('Status is not required')
        }
    ...
    check('destination')
      ...
      .custom((value) => isArrayOfValidStrings(value, 'Destination should be an array of destination locations'))
      .custom((value, { req }) => {
    ...
    check('return_date')
      ...
      .custom((value, { req }) => {
        ...
        if (category === ONE_WAY && value) {
          ...
          if ((!value && req.body.category) && req.existingRequest.category === ONE_WAY && req.body.category !== ONE_WAY) {
