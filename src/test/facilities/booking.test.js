...
const userToken = JWTService.generateToken({ id: 3, role: 'requester' });
const baseUrl = '/api/v1/facilities/rooms';

describe(`POST ${baseUrl}/:room_id/book`, () => {
  describe('SUCCESS', () => {
    it('should book an accommodation for a time not yet booked', async () => {
      const roomId = 2;
      ...
      expect(res.body.data).to.have.keys('id', 'user_id', 'room_id', 'facility_id',
      ...
      expect(res.body.data.return_date).to.equal(new Date(
        testBookingData.return_date
      ).toISOString().split('T')[0]);
    });

    it('should book same accommodation for a different time and interval', async () => {
      const roomId = 2;
      const testBookingData = {
          facility_id: 1,
  ...
  describe('FAILURE', () => {
    it('should fail to book an accommodation when token not provided', async () => {
      const roomId = 2;
      const res = await chai.request(app)
        .post(`${baseUrl}/${roomId}/book`)
        .send({
    ...
    it('should fail to book an accommodation when invalid token is provided',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation with a valid token for which user exist not in the db',
      async () => {
    ...
    it('should fail to book an accommodation with no departure date provided',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
      ...
      it('should fail to book an accommodation with no return date provided',
          async () => {
              const roomId = 2;
              const res = await chai.request(app)
                  .post(`${baseUrl}/${roomId}/book`)
      ...
      it('should fail to book an accommodation when invalid departure date is provided',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation when invalid return date is provided',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation when departure and return dates are same',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation when departure date is ahead of return date',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation when departure date is less than today',
      async () => {
        const roomId = 2;
        const res = await chai.request(app)
          .post(`${baseUrl}/${roomId}/book`)
    ...
    it('should fail to book an accommodation when referenced room does not exist',
      async () => {
        const res = await chai.request(app)
    ...
    it(`should fail to book an accommodation when both departure and return dates sought for a 
    ...
    async () => {
      ...
      expect(res.body.error.message.startsWith('This accommodation is already book from'))
        .to.be.true;
    });

    it(`should fail to book an accommodation when both departure and return dates for an 
    ...
    it(`should fail to book an accommodation when only the departure date sought for a new 
    ...
    it(`should fail to book an accommodation when only the return date sought for a new 
    booking falls within an existing booking\'s dates for same room`,
    async () => {
      const roomId = 2;
      const res = await chai.request(app)
